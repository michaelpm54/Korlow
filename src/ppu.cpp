...
Ppu::Ppu(PpuRegisters registers)
    : registers(registers), memory(0x2000), oam(0x100), pixels(160 * 144) {}

const uint8_t *Ppu::get_pixels() const { return pixels.data(); }
...
void Ppu::draw_scanline(int line) {
  ...
  if (sprites_dirty) {
    ...
    std::sort(
        ...
        [](const sprite_t &lh, const sprite_t &rh) { return lh.x < rh.x; });
